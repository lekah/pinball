...
MODULE becmod_flipper
  ...
  USE pinball,          ONLY : nr_of_pinballs
  !
  SAVE
  !
  TYPE bec_type
...
CONTAINS
  ...
  SUBROUTINE calbec_gamma ( npw, beta, psi, betapsi, nbnd, comm )
    ...
    ELSE
        !
        CALL DGEMM( 'C', 'N', nr_of_pinballs*nh(1), m, 2*npw, 2.0_DP, beta, 2*npwx, psi, &
                    2*npwx, 0.0_DP, betapsi, nkb )
        IF ( gstart == 2 ) &
           CALL DGER( nr_of_pinballs*nh(1), m, -1.0_DP, beta, 2*npwx, psi, 2*npwx, betapsi, nkb )
        !
     ENDIF
     !
