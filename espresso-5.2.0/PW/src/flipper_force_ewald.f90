...
!
!-----------------------------------------------------------------------
!subroutine flipper_force_ewald (alat, nat, nr_of_pinballs, ntyp, ityp, zv, at, bg, tau, &
!     omega, g, gg, ngm, gstart, gamma_only, gcutm, strf, forceion)
subroutine flipper_force_ewald (alat, nat, ntyp, ityp, zv, at, bg, tau, &
     omega, g, gg, ngm, gstart, gamma_only, gcutm, strf)
  !-----------------------------------------------------------------------
  !
...
!  integer :: nat, nr_of_pinballs, ntyp, ngm, ityp (nat), gstart
  ...
  !
!  real(DP) :: forceion (3, nr_of_pinballs)
...
10 alpha = alpha - 0.1d0
  ...
  IF ( do_comp_esm .and. ( esm_bc .ne. 'pbc') ) THEN
     ...
     ! 
     CALL esm_force_ew ( alpha, flipper_ewald_force )
     !
  ELSE
     allocate(aux(ngm))
     aux(:) = (0.d0, 0.d0)
     ...
     do na = 1, nr_of_pinballs
        do ig = gstart, ngm
           ...
           flipper_ewald_force (1, na) = flipper_ewald_force (1, na) + g (1, ig) * sumnb
           flipper_ewald_force (2, na) = flipper_ewald_force (2, na) + g (2, ig) * sumnb
           flipper_ewald_force (3, na) = flipper_ewald_force (3, na) + g (3, ig) * sumnb
        enddo
        do ipol = 1, 3
           flipper_ewald_force (ipol, na) = - zv (ityp (na) ) * fact * e2 * tpi**2 / &
                omega / alat * flipper_ewald_force (ipol, na)
        enddo
     enddo
     deallocate (aux)
  ENDIF
...
100 continue
  !
  CALL mp_sum( flipper_ewald_force, intra_bgrp_comm )
  !
  return
end subroutine flipper_force_ewald
