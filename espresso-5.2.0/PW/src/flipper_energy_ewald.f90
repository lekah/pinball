...
!
!-----------------------------------------------------------------------
function flipper_energy_ewald (alat, nat, ntyp, ityp, zv, at, bg, tau, omega, g, &
...
100 alpha = alpha - 0.1d0
  ...
  if (gstart.eq.2) then
     ...
     !
     do na = 1, nat
        if (flipper_ewald_list(ityp(na))) THEN
            do nb = 1, nat
  ...
  flipper_energy_ewald = 0.5d0 * e2 * (ewaldg + ewaldr)
  ! LEONID commented next oart (martyna tuckerman?)
  ! if ( do_comp_mt ) ewald =  ewald + wg_corr_ewald ( omega, ntyp, ngm, zv, strf )
  !
  call mp_sum(  flipper_energy_ewald, intra_bgrp_comm )
  ...
  !     + alpha, ewaldr, ewaldg
  return
end function flipper_energy_ewald
